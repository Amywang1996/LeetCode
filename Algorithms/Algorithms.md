...
# 9. 回文数
<B>题目</B><br/>
给你一个整数x,如果x是一个回文整数,返回true;否则返回false 。<br/>
...
- -2^31 <= x <= 2^31 - 1<br/>
<B>分析：</B> <br/>
第一步：第一想法是把数字转换成字符串，并检查字符串是否是回文。但是，这需要额外的非常量空间来创建问题描述中所不允许的字符串。
第二步：第一步的方法并不好，第二个想法是将数字本身反转，然后将反转的数字与原数字进行比较，如果它们相同，就是回文数。
第三步：第二步可能会有整数溢出问题，按照第一步想到了更好的方法：将数字反转一半做对比。
<B>Java代码：时间复杂度：O(log10(n));空间复杂度：O(1)</B>  
```java
class Solution {
    public boolean isPalindrome(int x) {
      //x为负数时,x不是回文数
      //x的末尾数字是0时，只有x=0才满足x是回文数的要求
      if(x<0||(x%10==0&&x!=0)){
          return false;
      }
