...
自关联查询，关联条件为s2.score>=s1.score，用s1.score查分数列，s2.score查询名次列，需要根据id分组，不然查出的数据只有表数据的第一条分数，以及它所对应的名次。
...
第一步，定义一个用户变量pre记录前一个的数据(第一条数据的前一个数据为null)，一个变量times记录数字出现的次数，如果当前num的值等于前一个变量，说明这个num重复出现了，次数加1，把当前的num赋值给pre变量，作为下一个num的前一个变量。

**结论：**

查询表用if判断num与pre是否相等，相等则times+1,否则times=1表示一次出现，筛选出times>=3的数据列表。
...
FROM
    ...
    SELECT
        ...
        @times :=
    IF
        ( @pre = num, @times + 1, 1 ) AS times,
        @pre := num AS pre 
    FROM
        LOGS,
        ( SELECT @pre := NULL, @times := 0 ) v 
	) l 
WHERE
	l.times >=3
```
